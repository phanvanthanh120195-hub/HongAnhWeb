$(document).ready(function () {
    ...
    // Check if device is iPad or Mobile (width <= 1024px)
    var isTabletOrMobile = window.innerWidth <= 1024;

    if (isTabletOrMobile) {
        // On iPad/Mobile: Show all content immediately without scroll animations
        // This prevents content from being hidden due to animation triggers
        $('#showcase h1, #showcase h2, #showcase p').css({ 'opacity': 1, 'letter-spacing': 'normal' });
        $('#showcase-2 h1, #showcase-2 h2, #showcase-2 p').css({ 'opacity': 1, 'letter-spacing': 'normal' });
        $('#showcase-3 h1, #showcase-3 h2, #showcase-3 p').css({ 'opacity': 1, 'letter-spacing': 'normal' });
        $('#showcase-4 h1, #showcase-4 h2, #showcase-4 p').css({ 'opacity': 1, 'letter-spacing': 'normal' });
        $('#showcase .gallery figure, #showcase-2 .gallery figure, #showcase-3 .gallery figure, #showcase-4 .gallery figure').css({ 'opacity': 1, 'position': 'relative', 'top': 0, 'left': 0 });
    } else {
        // On Desktop: Use scroll animations as normal
        ...
        function initScrollAnimations() {
            ...
            $('#showcase .gallery figure').css('position', 'relative').each(function () {
                controller.addTween('#showcase .gallery', TweenMax.from($(this), 1, {
                    delay: Math.random() * .2, css: { left: 0, top: Math.random() * 200 - 100, opacity: 0 },
                    ease: Back.easeOut
                }));
            });
            // showcase tweens 2
            ...
            $('#showcase-2 .gallery figure').css('position', 'relative').each(function () {
                controller.addTween('#showcase-2 .gallery', TweenMax.from($(this), 1, {
                    delay: Math.random() * .2, css: { left: 0, top: Math.random() * 200 - 100, opacity: 0 },
                    ease: Back.easeOut
                }));
            });
            ...
            $('#showcase-3 .gallery figure').css('position', 'relative').each(function () {
                controller.addTween('#showcase-3 .gallery', TweenMax.from($(this), 1, {
                    delay: Math.random() * .2, css: { left: 0, top: Math.random() * 200 - 100, opacity: 0 },
                    ease: Back.easeOut
                }));
            });
            ...
            $('#showcase-4 .gallery figure').css('position', 'relative').each(function () {
                controller.addTween('#showcase-4 .gallery', TweenMax.from($(this), 1, {
                    delay: Math.random() * .2, css: { left: 0, top: Math.random() * 200 - 100, opacity: 0 },
                    ease: Back.easeOut
                }));
            });
